...
use chrono::Utc;
use nom::{self, rest, IResult, InputIter, InputTake, be_u16, be_u32, be_u8};
use num_traits::FromPrimitive;
use std::collections::HashMap;
use super::{CompressionAlgorithm, RevocationCode, Subpacket};
use super::keys::PublicKeyAlgorithm;
use super::hash::HashAlgorithm;
use super::symmetric::SymmetricKeyAlgorithm;
use super::util::{clone_into_array, mpi, packet_length};
use ex_crypto::error::ErrorKind;
use std::str;
use ex_crypto::openpgp::packet::SubpacketType;
...
named!(
    ...
    do_parse!(
        algs: many0!(complete!(map_opt!(be_u8, HashAlgorithm::from_u8))) >> (Subpacket::PreferredHashAlgorithms(algs))
    )
);
...
named!(
    ...
    do_parse!(
        class: be_u8 >> alg: map_opt!(be_u8, PublicKeyAlgorithm::from_u8) >> fp: take!(20)
...
named!(
    ...
    do_parse!(
        // Flags
        tag!(&[0x80, 0, 0, 0][..]) >> name_len: be_u16 >> value_len: be_u16
...
named!(
    ...
    do_parse!(body: map_res!(rest, str::from_utf8) >> ({ Subpacket::PreferredKeyServer(body.to_string()) }))
);
...
named!(
    ...
    do_parse!(
        code: map_opt!(be_u8, RevocationCode::from_u8) >> reason: rest
...
named!(
    ...
    >>      sig: complete!(call!(actual_signature, &pub_alg)) >> ({
            ...
            for p in hsub {
                match p {
                    Subpacket::SignatureCreationTime(d) => sig.created = Some(d),
                    Subpacket::Issuer(a) => sig.issuer = Some(a),
                    Subpacket::PreferredSymmetricAlgorithms(list) => sig.preferred_symmetric_algs = list,
                    Subpacket::PreferredHashAlgorithms(list) => sig.preferred_hash_algs = list,
                    Subpacket::PreferredCompressionAlgorithms(list) => sig.preferred_compression_algs = list,
                    Subpacket::KeyServerPreferences(f) => sig.key_server_prefs = f,
                    Subpacket::KeyFlags(f) => sig.key_flags = f,
                    Subpacket::Features(f) => sig.features = f,
                    Subpacket::RevocationReason(code, body) => {
                        sig.revocation_reason_code = Some(code);
                        sig.revocation_reason_string = Some(String::from_utf8_lossy(body.as_slice()).to_string());
                    }
                    Subpacket::IsPrimary(b) => sig.is_primary = b,
                    Subpacket::KeyExpirationTime(d) => sig.key_expiration_time = Some(d),
                    Subpacket::Revocable(b) => sig.is_revocable = b,
                    Subpacket::EmbeddedSignature(mut inner_sig) => sig.embedded_signature = Some(inner_sig),
                    Subpacket::PreferredKeyServer(server) => sig.preferred_key_server = Some(server),
                    Subpacket::SignatureExpirationTime(d) => sig.signature_expiration_time = Some(d),
                    Subpacket::Notation(name, value) => {
                        sig.notations.insert(name, value);
                    }
                    Subpacket::RevocationKey(class, algorithm, fingerprint) => {
                    ...
                    Subpacket::SignersUserID(u) => sig.signers_userid = Some(u),
                    Subpacket::PolicyURI(s) => sig.policy_uri = Some(s),
                    Subpacket::TrustSignature(v) => sig.trust_signature = Some(v),
                    Subpacket::RegularExpression(v) => sig.regular_expression = Some(v),
                    Subpacket::ExportableCertification(v) => sig.exportable_certification = v,
                }
            }
