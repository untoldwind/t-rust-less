...
impl ClipboardCommon for Clipboard {
  ...
  {
    match unix_wayland::Clipboard::new(display_name, selection_provider.clone(), event_hub.clone()) {
      Ok(wayland) => Ok(Clipboard::Wayland(wayland)),
      Err(ClipboardError::Unavailable) => {
        info!("Wayland unavailable, fallback to x11");
        unix_x11::Clipboard::new(display_name, selection_provider, event_hub).map(Clipboard::X11)
      }
      Err(err) => Err(err),
    }
  }
